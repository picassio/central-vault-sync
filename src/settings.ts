...
export class CentralSyncSettingTab extends PluginSettingTab {
          ...
          { name: 'Device name', desc: 'Shown in the server device list and conflict-copy names.', control: { type: 'text', key: 'deviceName' } },
          {
            ...
            render: (setting) => {
              ...
              setting.addButton((button) => {
                button.setButtonText('Unpair').setDisabled(!state.deviceId).onClick(async () => { await this.controller.unpair(); this.refresh(); });
          ...
          },
          {
            ...
            render: (setting) => {
              ...
              setting.addButton((button) => button.setCta().setButtonText('Pair').setDisabled(Boolean(state.deviceId)).onClick(async () => {
                try { await this.controller.pair(code); new Notice('Central sync paired'); this.refresh(); }
          ...
          { name: 'Server-enforced exclusions', desc: `${this.app.vault.configDir}/**, .git/**, .trash/**, temporary/OS files, and internal sync metadata never synchronize.` },
          {
            ...
            render: (setting) => {
              setting.addButton((button) => button.setCta().setButtonText('Sync now').setDisabled(!state.deviceId || state.paused).onClick(async () => {
                try { await this.controller.syncNow(); new Notice('Central sync complete'); this.refresh(); }
                catch (error) { new Notice(message(error)); }
              }));
  ...
  display(): void {
    ...
    for (const item of this.getSettingDefinitions()) {
      ...
      for (const definition of item.items ?? []) {
        if (isNestedItem(definition) || !isVisible(definition)) continue;
        const setting = new Setting(this.containerEl).setName(definition.name);
        if (definition.desc) setting.setDesc(definition.desc);
        if ('render' in definition && definition.render) definition.render(setting, undefined as never);
        else if ('control' in definition && definition.control) this.renderLegacyControl(setting, definition.control);
      }
    }
  }

  private renderLegacyControl(setting: Setting, control: SettingControl): void {
    const value = this.getControlValue(control.key);
    if (control.type === 'toggle') {
      setting.addToggle((toggle) => toggle.setValue(Boolean(value)).onChange((next) => void this.setControlValue(control.key, next)));
    } else if (control.type === 'textarea') {
      setting.addTextArea((text) => text.setValue(controlString(value)).onChange((next) => void this.setControlValue(control.key, next)));
      ...
      setting.addText((text) => {
        ...
        text.setValue(controlString(value ?? control.defaultValue)).onChange((next) => void this.setControlValue(control.key, next));
      });
  ...
  private refresh(): void {
    const modernUpdate = (this as unknown as { update?: () => void }).update;
    if (modernUpdate) modernUpdate.call(this);
    else (this as unknown as { display: () => void }).display();
  }
}
...
function isNestedItem(item: SettingDefinition | SettingDefinitionItem): item is Exclude<SettingDefinitionItem, SettingDefinition> {
  return 'type' in item;
}
function isVisible(item: SettingDefinition): boolean {
